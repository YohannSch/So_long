...
/*   Updated: 2025/12/04 11:15:55 by yscheupl         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line.h"

void	gnl_bzero(char *str, int size)
{
	int	i;
...
char	*fill_the_line(char *temp, char *newline_pos, char *buffer)
{
	...
	while (temp[len] && temp[len] != '\n')
		len++;
	if (temp[len] == '\n')
		res = gnl_substr(temp, 0, len + 1);
	else
		res = gnl_substr(temp, 0, len);
	newline_pos++;
	while (*newline_pos)
